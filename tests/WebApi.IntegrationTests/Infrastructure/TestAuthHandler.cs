...
/// authenticated; the user id comes from the <c>X-Test-UserId</c> header (a GUID) so a test can act
/// as an arbitrary user. This satisfies <c>RequireAuthorization()</c> and the modules'
/// <c>UserIdEndpointFilter</c>, which reads <see cref="ClaimTypes.NameIdentifier"/> and parses it as a Guid.
/// </summary>
    ...
    UrlEncoder encoder) : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        // An explicitly non-GUID header lets tests exercise the "authenticated but no valid user id" path.
