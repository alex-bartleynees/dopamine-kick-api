...
/// Building the host triggers the app's start-up migrations, so all four module schemas exist by the
/// time any test runs.
/// </summary>
public sealed class ApiTestFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        // Force host construction (and therefore start-up migrations) before the first test.
    ...
    /// <summary>An HttpClient with a non-GUID id claim (authenticated, but no valid user id).</summary>
    public HttpClient CreateClientWithInvalidUserId()
    {
        var client = Factory.CreateClient();
