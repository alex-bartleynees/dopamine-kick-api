...
/// </summary>
public record CreateCheckoutSession(Guid UserId, string Email) : IRequest<Result<CheckoutSessionResponse>>;
    ...
    IPaymentGateway paymentGateway) : IRequestHandler<CreateCheckoutSession, Result<CheckoutSessionResponse>>
{
        ...
        CreateCheckoutSession request, CancellationToken cancellationToken)
    {
        ...
        var customerResult = await customerService.EnsureCustomerAsync(request.UserId, request.Email, cancellationToken);
        if (customerResult.IsFailure)
        {
            return Result<CheckoutSessionResponse>.Failure(customerResult.Error);
        }

        var url = await paymentGateway.CreateCheckoutSessionUrlAsync(customerResult.ValueOrThrow, cancellationToken);
