...
/// server-side from the authenticated user — never supplied by the client.
/// </summary>
public record CreatePortalSession(Guid UserId) : IRequest<Result<PortalSessionResponse>>;
    ...
    IPaymentGateway paymentGateway) : IRequestHandler<CreatePortalSession, Result<PortalSessionResponse>>
{
        ...
        CreatePortalSession request, CancellationToken cancellationToken)
    {
        ...
        var mapping = await repository.GetMappingByUserIdAsync(request.UserId, cancellationToken);
        if (mapping is null)
        {
            return Result<PortalSessionResponse>.Failure(PaymentsErrors.NoCustomer);
        }

        var url = await paymentGateway.CreatePortalSessionUrlAsync(mapping.CustomerReference, cancellationToken);
