...
{
    public async Task<Result<string>> EnsureCustomerAsync(Guid userId, string email, CancellationToken ct = default)
    {
        ...
        var existing = await repository.GetMappingByUserIdAsync(userId, ct);
        if (existing is not null)
        {
            return Result<string>.Success(existing.CustomerReference);
        }

        var customerReference = await paymentGateway.CreateCustomerAsync(userId, email, ct);

        await repository.AddMappingAsync(new CustomerMapping(userId, customerReference), ct);
