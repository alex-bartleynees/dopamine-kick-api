...
/// Stripe SDK types are used, translating them into the domain's provider-agnostic models. All config
/// (price, URLs, trial length, webhook secret) is read from <see cref="StripeOptions"/> so callers only
/// ever pass an opaque customer reference.
/// </summary>
public class StripePaymentGateway : IPaymentGateway
{
    ...
    public async Task<string> CreateCheckoutSessionUrlAsync(string customerReference, CancellationToken ct = default)
    {
        var options = new Stripe.Checkout.SessionCreateOptions
        {
            ...
            LineItems =
            [
                new Stripe.Checkout.SessionLineItemOptions { Price = _options.PriceId, Quantity = 1 }
            ],
            SuccessUrl = _options.SuccessUrl,
            CancelUrl = _options.CancelUrl,
            SubscriptionData = _options.TrialPeriodDays > 0
                ? new Stripe.Checkout.SessionSubscriptionDataOptions { TrialPeriodDays = _options.TrialPeriodDays }
                : null
        };
    ...
    public async Task<string> CreatePortalSessionUrlAsync(string customerReference, CancellationToken ct = default)
    {
